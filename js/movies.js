...
async function get() {
  ...
  const response = await fetch(`https://api.themoviedb.org/3/trending/movie/day?api_key=${apikey}&page=${dayPage}`);
...
async function getWeek() {
  ...
  const response = await fetch(`https://api.themoviedb.org/3/trending/movie/week?api_key=${apikey}&page=${weekPage}`);
  const data = await response.json();
  show(data.results);
}
...
async function searchByText(searchText) {
  const moviesBySearch = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=${apikey}&query=${searchText}&page=${searchPage}`);
...
async function showMoviesBySearch(id) {
  ...
  card = `<div onclick= 'showDetails(${modalData.id}, true)' class="card" style="width: 18rem;">
        <img src="${modalData.poster_path ? base_url + modalData.poster_path : "images/profile (1).png"}" class="card-img-top" alt="${modalData.title}">
        <div class="card-body">
          <h5 id="title">${modalData.title}</h5>
